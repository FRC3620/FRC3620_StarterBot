...
public final class Minifier {
	...
	/**
	 * Remove single and multi-line comments from any valid java/javascript/json string
	...
	public static final String minify(String json) {
		...
		for (int i = 0; i < json.length(); i++) {
			...
			if (currentStringChar != null) {
				sb.append(current);
				if (current == STRING_CHARS.get(currentStringChar)) currentStringChar = null;
				if (ESCAPE_CHARS.contains(current) && json.length() > ++i) sb.append(json.charAt(i));
				...
				if (STRING_CHARS.containsKey(current)) currentStringChar = current;
				else {
					char lookup;
					lookup: {
						for (String opener : COMMENT_INDICATORS.keySet()) {
							...
							for (int x = 0; x < opener.length(); x++) {
								...
								if (lookup == opener.charAt(x)) {
									if (indicator == null) indicator = new StringBuilder();
